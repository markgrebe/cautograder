...
for file in zipdir.glob('*.zip'):
    ...
    # For each of the compile flags in Question 3's test set.
    flagnum = 0
    flag_array = ["-D FIRST", "-D SECOND", "-D FIRST -D SECOND"]
    for flags in flag_array:
        # Compile the program
        os.system("clang -o " + student_dir + "/cond " + flags + " " + 
                  student_dir + "/cond.c" +  
                  " 2> " + student_dir + "/cond.compile" + str(flagnum))

        myoutput = open(student_dir + "/cond" + ".output" + str(flagnum), 'w')
        if os.path.isfile(student_dir + "/cond"):
            p = subprocess.Popen(student_dir + "/cond", stdout=myoutput)
            try:
                p.wait(PROGTIMEOUT)
            except:
                myoutput.write("****** Process Timeout: cond\n")
                p.terminate()
        else:
            myoutput.write("****** Executable Not Found: cond\n")
        myoutput.flush()
        myoutput.close()
        flagnum = flagnum + 1
